The Pima Indians Diabetes project is a data science pipeline / machine learning model developed as a capstone project .
This project demonstrates proficiency in [relevant skill areas, e.g., RESTful API design, database modeling, responsive UI development, or machine learning] and was built following industry best practices for code quality, documentation, and deployment.

 The dataset has 768 samples and the task is to 	Predict the onset of diabetes based on medical and demographic data such as glucose levels, BMI, and age. The data was imported and the missing valuses were checked.  The data has 768 rows and 9 columns. There is no duplicated value.
